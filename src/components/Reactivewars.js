    ...
    getDataStream
      .pipe(
        flatMap(val => {
          ...
          let outVal = val;
          return axios.get(
            `/${val.pref}/?search=${val.searchText}`
          )
            .then(val => val.data)
            //.then(val => of({ pref: outVal.pref, res: val.results }))// Pasing data downstream for later use
            .then(val=> this.setState({data:{...this.state.data,[outVal.pref]: {
                count: val.count,
                dataArray: val.results
              }}}))
        }),
        catchError(err => {
          return empty();
